 ...
 * @LastEditors: zhangxi
 * @LastEditTime: 2020-06-05 11:10:22
 -->
<template>
  <div class="security-container">
    <!-- <div class="header-bg">安全态势安全生产</div> -->
    <head-componen name="安全态势安全生产"></head-componen>
    ...
    <div class="main-left-center">
      <p class="g-title">近五年安全生产事故数量TOP6</p>
      ...
      <div class="accident-area-rank">
        <div class="rank-item" v-for=" (item,index) in accidentDistrictList" :key="index">
          ...
          <div class="list-no">{{index+1}}</div>
          <div class="accident-rank-desc">{{item.district}}</div>
          ...
          <div class="accident-rank-num">{{item.thingSum}}起</div>
        </div>
      </div>
    </div>
    <div class="main-left-bottom">
      <p class="g-title">近五年安全生产事故影响TOP5类型</p>
      <div class="select">
      ...
      <div id="accidentTypeChart"></div>
    </div>
    <div class="main-left-top main-right-top">
      <p class="g-title">近五年安全生产事故影响及数量趋势</p>
      <div class="select">
      ...
      <div id="accidentTrendChart"></div>
    </div>
    <div class="main-left-center main-right-center">
      <p class="g-title">安全生产事故月度分析</p>
      <div id="accidentTotalChart"></div>
      <div class="select">
    ...
    <div class="main-left-bottom main-right-bottom">
      <p class="g-title">经济损失TOP10安全生产事故</p>
      <div class="time-container">
        <ul class="timeline">
          <li v-for="(item,index) in accidentEcoList" :key="index">
            <p class="timestamp">{{item.yearDate}}</p>
            ...
            <div class="details">
              <div class="title">
                ...
                <span class="number">
                  经济损失：
                  <big>{{item.ecoNum}}</big>万元
                </span>
              </div>
              <div class="content" @mouseenter="showTooltip($event)" @mouseleave="hideTooltip">
                {{item.details}}
      ...
      <div class="select">
        ...
        <ul v-if="select2IsShow" class="select-list">
          <li
            ...
            :key="index"
            @click="selectItemClickFn2(item.years)"
          >{{item.years}}年</li>
        </ul>
      </div>
    </div>
      ...
      <!-- <div class="maker-text" v-if="item.showTextFlag&&isShowMiniMarker===1">
        ...
        <div class="marker-info-box">
          ...
          <div class="content">
            ...
            <div class="loss">
              经济损失：
              <span>{{(thingDetailsMap&&thingDetailsMap.ecoNum)+"万元"||'无数据'}}</span>
            </div>
            <div class="details">
              详细内容:
          ...
          <div class="input-con select">
            ...
            <ul class="select-container" v-if="typeList.length>0" v-show="isShowTypeList">
              <li
                ...
                :class="{active:searchObj.type===item.sc02003}"
              >{{item.sc02003}}</li>
            </ul>
          </div>
          <div class="input-con select">
      ...
      >
        ...
        >
          ...
          <div class="marker-detail">
            <p class="title">近五年安全生产事故TOP5类型</p>
            <div class="info-list">
              <ul>
                <template v-if="item.top5.length>0">
                  <li v-for="(m,n) in item.top5" :key="n">
                    ...
                    <span>{{m.sc02003}}</span>
                    <span>{{m.num}}</span>
                    <span @click="openHarzdDialog(item.qxq,m.sc02003)">>></span>
                  </li>
                </template>
                <template v-else>
                  <li>无数据</li>
...
import accidentTypeChartOptions from '@/echarts-data/emergency-command/overall-situation/security-situation/accidentTypeChart.js'
import accidentTrendChartOptions from '@/echarts-data/emergency-command/overall-situation/security-situation/accidentTrendChart.js'
import accidentTotalChartOptions from '@/echarts-data/emergency-command/overall-situation/security-situation/accidentTotalChart.js'
import API from '@/api/emergency-command/security-situation'
...
export default {
  ...
  methods: {
    ...
    selectItemClickFn1 (year) {
      ...
      API.ACCIDENT_YEAR(this, { year: this.activeYear1 }).then(res => {
        ...
        if (res.serviceSuccess) {
          accidentTotalChartOptions.xAxis[0].data =
            res.data.accidentYearMap.monthList
          accidentTotalChartOptions.series[0].data =
            res.data.accidentYearMap.thingSumList
          accidentTotalChartOptions.series[1].data =
            res.data.accidentYearMap.ecoSumList
          //  console.log('accidentTotalChartOptions', accidentTotalChartOptions)
          echarts
            .init(document.querySelector('#accidentTotalChart'))
            .setOption(accidentTotalChartOptions)
        }
      })
    },
    select1Click () {
    ...
    initLeftBottomChart (index) {
      switch (index) {
        case '经济损失':
          accidentTypeChartOptions.series[1].name = '经济损失'
          accidentTypeChartOptions.series[1].data = this.accidentTypeTop5Map.ecoSumList
          accidentTypeChartOptions.legend.data[1] = '经济损失'
          accidentTypeChartOptions.yAxis[1].name = '(万元)'
          break
        case '伤亡数量':
          accidentTypeChartOptions.series[1].name = '伤亡数量'
          accidentTypeChartOptions.series[1].data = this.accidentTypeTop5Map.deathNumList
          accidentTypeChartOptions.legend.data[1] = '伤亡数量'
          accidentTypeChartOptions.yAxis[1].name = '(人)'
          break
      }
      echarts
        .init(document.querySelector('#accidentTypeChart'))
        .setOption(accidentTypeChartOptions)
    },
    initRightTopChart (index) {
      switch (index) {
        case '经济损失':
          accidentTrendChartOptions.series[1].name = '经济损失'
          accidentTrendChartOptions.series[1].data = this.accidentTrend5YearsMap.ecoSumList
          accidentTrendChartOptions.legend.data[1] = '经济损失'
          accidentTrendChartOptions.yAxis[1].name = '(万元)'
          break
        case '伤亡数量':
          accidentTrendChartOptions.series[1].name = '伤亡数量'
          accidentTrendChartOptions.series[1].data = this.accidentTrend5YearsMap.deathList
          accidentTrendChartOptions.legend.data[1] = '伤亡数量'
          accidentTrendChartOptions.yAxis[1].name = '(人)'
          break
      }

      echarts
        .init(document.querySelector('#accidentTrendChart'))
        .setOption(accidentTrendChartOptions)
    },
    getTimelineData () {
      API.ACCIDENT_ECO(this, { year: this.activeYear2 }).then(res => {
        res = res.data
        if (res.serviceSuccess) {
          this.accidentEcoList = res.data.accidentEcoList
        }
      })
    },
    // 获取年份列表
    getYearList () {
      // 获取安全生产事故月度分析年份列表
      API.QUERY_YEARS(this, {code: '01'}).then(res => {
        let re = res.data
      ...
      // 获取经济损失top10安全事故年份列表
      API.QUERY_YEARS(this, {code: '02'}).then(res => {
        let re = res.data
        if (re.serviceSuccess) {
          this.days = re.data.queryYears
          this.activeYear2 = this.days[0].years
          this.selectItemClickFn2(this.activeYear2)
        }
      })
    },
    // 鼠标移入显示标题
    ...
    getData () {
      ...
      API.ACCIDENT_TYPE(this, {}).then(res => {
        ...
        if (res.serviceSuccess) {
          ...
          accidentTypeChartOptions.series[0].data = temp
          accidentTypeChartOptions.xAxis[0].data = this.accidentTypeTop5Map.thingTypeList
      ...
      API.ACCIDENT_TREND(this, {}).then(res => {
        ...
        if (res.serviceSuccess) {
          ...
          accidentTrendChartOptions.series[0].data = this.accidentTrend5YearsMap.thingSumList
          accidentTrendChartOptions.xAxis[0].data = this.accidentTrend5YearsMap.yearList
      ...
      // this.getOverAllSpread(3)
    },
    ...commonFn,
    /***
     * @description mini marker 点击事件  切换到3D地图
     */
    handleClearTime () {
      if (this.qxTimer) {
    ...
    typeListSelect (item) {
      ...
      this.searchObj.type = item.sc02003
    },
    qxListSelect (item) {
    ...
    queryHealthInfo () {
      ...
      API.QUERY_PRODUCTION_INFO(this, this.searchObj).then(res => {
        res = res.data
        if (res.serviceSuccess) {
    ...
    signDetailsPoint (item) {
      ...
      this.detailsPoint = {
        ...
        lat: item.lat,
        type: item.sc02003,
        district: item.sc02007,
        time: item.sc02001,
        details: item.sc02006,
        id: 'production_detailsPoint'
      }
      this.handleClearTime()
    ...
    detailsPointClose () {
      this.detailsPoint = {}
      window._m && window._m.removeMaker(['production_detailsPoint'], true)
      this.handleMove()
    }
  },
  mounted () {
  ...
  li {
    ...
    .details {
      ...
      .content {
        ...
        -webkit-line-clamp: 3;
        overflow: hidden;
        background: url("../../../../assets/emergency-command/overall-situation/security-situation/timeline-text-bg.png");
        background-size: 100% 100%;
      }
    }
  }
