 ...
 * @Descripttion:
 * @Author: hangzhang
 * @Date: 2019-12-15 16:44:12
 * @LastEditors  : hangzhang
 * @LastEditTime : 2019-12-20 09:53:42
 */
// 数字滚动
    ...
    scroNum: function (obj, options) {
      ...
      $num_item.each(function (i, item) {
        setTimeout(function () {
          if (numberArr[i] === ',') {
            $num_item.eq(i).html(',')
            $num_item.eq(i).css({'font-size': '25px', 'background': 'none', 'transition': 'none', 'color': '#fff5a1', 'font-family': '微软雅黑'})
          }
          if (numberArr[i] === '.') {
            $num_item.eq(i).html('.').css({'font-size': '25px', 'background': 'none', 'transition': 'none', 'color': '#fff5a1', 'font-family': '微软雅黑'})
            // $num_item.eq(i)
          }
