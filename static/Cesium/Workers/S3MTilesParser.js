...
define(['./when-a55a8a4c', './Check-bc1d37d9', './Math-edfe2d1c', './Cartesian2-f0158650', './BoundingSphere-02d3af5e', './RuntimeError-7c184ac0', './WebGLConstants-4c11ee5f', './ComponentDatatype-919a7463', './PrimitiveType-97893bc7', './FeatureDetection-bac17d71', './createTaskProcessorWorker', './BoundingRectangle-b42c500a', './Color-b1821df1', './pako_inflate-8ea163f9', './S3MCompressType-a4eff709'], function (when, Check, _Math, Cartesian2, BoundingSphere, RuntimeError, WebGLConstants, ComponentDatatype, PrimitiveType, FeatureDetection, createTaskProcessorWorker, BoundingRectangle, Color, pako_inflate, S3MCompressType) { 'use strict';
    ...
    function parseGeoPackageDraco(geoPackage, dataBuffer, view, bytesOffset, transferableObjects) {
        var geoName = view.getUint32(bytesOffset, true);
        bytesOffset += Uint32Array.BYTES_PER_ELEMENT;
        var vertexPackage = {};
        vertexPackage.instanceMode = 0;
        ...
        var nAttriSize = view.getUint32(bytesOffset, true);
        bytesOffset += Uint32Array.BYTES_PER_ELEMENT;
        for (var i = 0; i < nAttriSize; i++) {
            var nAttriID = view.getUint32(bytesOffset, true);
            bytesOffset += Uint32Array.BYTES_PER_ELEMENT;
            var nAttriType = view.getUint32(bytesOffset, true);
            bytesOffset += Uint32Array.BYTES_PER_ELEMENT;
            attriDefines[nAttriType] = nAttriID;
        }

        var bufferLength = view.getUint32(bytesOffset, true);
        bytesOffset += Uint32Array.BYTES_PER_ELEMENT;
        var bufferOffset = bytesOffset;
        bytesOffset += bufferLength;

        var copyTypedArray = new Uint8Array(dataBuffer, bufferOffset, bufferLength);
        var typedArray = copyTypedArray.slice(0, bufferLength);
        transferableObjects.push(typedArray.buffer);

        var arrIndexPackage = [];
        var indexPackageCount = view.getUint32(bytesOffset, true);
        bytesOffset += Uint32Array.BYTES_PER_ELEMENT;
        for (var i = 0; i < indexPackageCount; i++) {
            var indexPackage = {};
            var indicesCount = view.getUint32(bytesOffset, true);
            bytesOffset += Uint32Array.BYTES_PER_ELEMENT;
            var indexType = view.getUint8(bytesOffset, true);
            ...
            indexPackage.indexType = indexType;
            indexPackage.primitiveType = primitiveType;
            indexPackage.offsetInBytes = bytesOffset;
            var passNameCount = view.getUint32(bytesOffset, true);
            bytesOffset += Uint32Array.BYTES_PER_ELEMENT;
            var materialCode = view.getUint32(bytesOffset, true);
            bytesOffset += Uint32Array.BYTES_PER_ELEMENT * passNameCount;
        ...
        geoPackage[geoName] = {
            ...
            arrIndexPackage: arrIndexPackage,
            array: typedArray,
            compressedAttributes: attriDefines,
    ...
    function S3MTilesParser(parameters, transferableObjects) {
        ...
        var compress = parameters.compress;
        ...
        for (var i = 0; i < geoCount; i++) {
            if (compress) {
                bytesOffset = parseGeoPackageDraco(geoPackage, unzipBuffer, view, bytesOffset, transferableObjects);
            }
            else {
